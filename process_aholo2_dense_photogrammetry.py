#!/usr/bin/env python3
"""
AHOLO 2.0 MULTI-VIEW STEREO DENSE SURFACE ENGINE
Version: v2.0.2

Projects 3D keypoints into 546 registered 4K camera keyframes to build 2D depth fields.
Triangulates dense surface points across rooflines, house facades, driveways, and tree canopies.
Samples 4K frame RGB colors directly for every surface pixel.

Output:
- aholo2_reconstruction.splat (Multi-million dense surface 3DGS binary buffer)
...
def main():
    ...
    # Subsample cameras (take 1 out of 3 frames = ~545 keyframes)
    sorted_cam_ids = sorted(cameras.keys())
    sampled_cam_ids = sorted_cam_ids[::3]
    log(f"Selected {len(sampled_cam_ids):,} keyframes for 2D-3D dense surface pixel triangulation...")
    ...
    grid_step = 24 # Extract ~3,600 surface pixels per keyframe -> ~2,000,000 dense surface points total!
    ...
    for cam_id in sampled_cam_ids:
        ...
        # Build 2D depth map for the image by sparse binning / median filter
        scale = 0.1 # 192x108 depth map
        ...
        # Filter out invalid depth / sky glare
        ...
        # Unproject pixels to 3D world space: P_cam = z * K^-1 [u, v, 1]^T -> P_world = R^T * (P_cam - t)
        ...
        if processed_frames % 100 == 0:
            log(f"Processed {processed_frames}/{len(sampled_cam_ids)} keyframes -> {len(dense_pts):,} total dense surface splats...")
    ...
    log(f"Aholo 2.0 Dense Photogrammetry completed in {time.time() - start_t:.2f}s total!")
